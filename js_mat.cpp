...
js_mat_get(JSContext* ctx, JSValueConst this_val, uint32_t row, uint32_t col, T& value) {
  ...
  value = m ? (*m).at<T>(row, col) : T();
}

template<class T, size_t N>
void
js_mat_get(JSContext* ctx, JSValueConst this_val, cv::Vec<int, N> idx, T& value) {
  cv::Mat* m = js_mat_data2(ctx, this_val);

  value = m ? (*m).at<T>(idx) : T();
}
...
static JSValue
js_mat_size_value(JSContext* ctx, cv::Mat* m) {
  JSValue ret;
...
js_mat_init(JSContext* ctx, JSModuleDef* m) {
  if(js_mat_class_id == 0) {
    ...
    js_object_inspect(ctx, mat_proto, js_mat_inspect);
...
JS_INIT_MODULE(JSContext* ctx, const char* module_name) {
  JSModuleDef* m;
  if(!(m = JS_NewCModule(ctx, module_name, &js_mat_init)))
    return NULL;
  js_mat_export(ctx, m);
  return m;
}

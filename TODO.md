...
*(Removed from this list, now fixed: `minMaxLoc` returned only a positional array, not opencv.js's named `{minVal, maxVal, minLoc, maxLoc}` object — `js_cv.cpp`'s `OTHER_MIN_MAX_LOC` now attaches the same four fields by name on top of the existing positional array, so both calling conventions work (checked no internal caller relied on `Array`-only shape; `test-image.js`/`test-video.js` in the sibling `plot-cv` repo were already using the named form and silently getting `undefined` — now fixed for free). `minEnclosingCircle` had no return value at all — `js_imgproc.cpp`'s `SHAPE_MIN_ENCLOSING_CIRCLE` now also returns a plain `{center, radius}` object (opencv.js's `Circle` is a plain object too, not a real class) alongside the pre-existing optional-callback form; updated `tests/unittests/test_contour_functions.js`'s `minEnclosingCircle - Mat CV_32SC2` test off its old (already-broken, since the old code returned `undefined`) `const [center, radius] = ...` destructure. `new Mat(otherMat)` (copy-handle constructor) didn't exist — `js_mat_initialize` (`js_mat.cpp`) now recognizes another `Mat` as `argv[0]` and placement-news a `cv::Mat` copy-constructed from it, sharing the reference-counted buffer like opencv.js's shallow-copy handle (`.dup()`, which wraps a fresh JS object instead of initializing `this`, is unchanged and still the way to get an independent copy). `ORB`/`MSER`/`AKAZE`/`BRISK`/`FastFeatureDetector`/`GFTTDetector` had no `.create()` static — `js_feature2d.cpp`'s per-class static-func tables now include a `create` entry pointing straight at the existing constructor-body function (none of them read `new_target`, so the same function serves both `new cv.ORB(...)` and `cv.ORB.create(...)`); `BRISK`/`MSER`/`GFTTDetector` previously had no static-func table at all and now get one just for `create`. `CamShift`/`meanShift` — `js_imgproc.cpp`'s new `js_imgproc_track` (magic dispatch, `TRACK_CAMSHIFT`/`TRACK_MEANSHIFT`) wraps `cv::CamShift`/`cv::meanShift`, returning `[RotatedRect, updatedRect]`/`[n, updatedRect]` and mutating the passed-in `Rect` in place, matching opencv.js's `core_bindings.cpp`. `criteria` follows this project's existing `kmeans` convention (a plain `[type, maxCount, epsilon]` array, not a `TermCriteria` object — see the still-open `kmeans` entry below for that broader gap). `cv.createCLAHE(clipLimit, tileGridSize)` free function — `js_clahe.cpp` now exports a module-level `createCLAHE` alongside `new CLAHE(...)`, matching opencv.js's whitelist (opencv.js exposes only the free-function form). `BarcodeDetector` naming — `js_barcode_detector.cpp:272-273,296-297` now exports both bare `BarcodeDetector` **and** `barcode_BarcodeDetector`, matching opencv.js's namespace-flattened name. `drawContours` with `index >= 0` breaking hierarchy-aware drawing — `js_draw_contours` (plural, `js_draw.cpp:220-264`) now threads the real `index`/`hier`/`maxLevel` straight into `cv::drawContours` against the full `contours` array/vector, no single-contour re-wrap. "No `cv.Range` class, array-shape-only" — `js_range.cpp`/`js_range.hpp` now provide a real constructible `cv.Range` class whose reader (`js_range_read`, `js_range.hpp:42-69`) accepts both the `[start,end]` array form and opencv.js's `{start,end}` object form.)*

...
- **`CamShift`/`meanShift`** — both are registered in opencv.js's `core_bindings.cpp` (returning `[RotatedRect, updatedRect]` and `[n, updatedRect]` arrays respectively). This project has no bindings for either; only the unrelated `pyrMeanShiftFiltering` exists (different algorithm). Ported tracking code calling `cv.CamShift(...)` or `cv.meanShift(...)` will throw `TypeError`.
- **`Mat.diag()`** — opencv.js binds both `diag(d)` (int offset) and `diag()` (no-arg, main diagonal). This project has no `diag` method at all on Mat.
- **`Mat.data` / `.data8S` / `.data16U` / `.data16S` / `.data32S` / `.data32F` / `.data64F`** — opencv.js exposes 7 typed-array views of the raw pixel buffer as properties (each a `memory_view<T>` of the appropriate element type). This project has `.buffer` (raw `ArrayBuffer`) and `.array` (row-major array of typed sub-arrays) instead, but none of the 7 named views. Code like `mat.data32F[i] = 0` silently reads `undefined`.
- **`Mat.charAt` / `ucharAt` / `shortAt` / `ushortAt` / `intAt` / `floatAt` / `doubleAt`** — opencv.js's per-type element accessors (each with 1/2/3-arg overloads for row, row+col, row+col+slice). This project has a generic `.at(row, col)` that infers type from `mat.type()`. Code using opencv.js's typed accessors throws `TypeError: mat.floatAt is not a function`.
- **`Mat.ucharPtr` / `charPtr` / `shortPtr` / `ushortPtr` / `intPtr` / `floatPtr` / `doublePtr`** — opencv.js's typed pointer accessors returning `memory_view<T>` of length `step1(0)` or `step1(1)`. This project's single `.ptr()` returns a typed array matching the Mat depth, but the 7 separate typed variants don't exist.
...
- **`Point(x)` / `Size(width)` with one argument throws instead of defaulting the second to 0.** opencv.js's constructors accept undefined defaults: `new Point(5)` → `{x:5, y:0}`. This project's constructors require exactly 2 arguments (`argc < 2` triggers early `JS_EXCEPTION` return). Ported code using single-arg constructors will crash.
- **`Scalar.all(v)` doesn't exist.** opencv.js's `Scalar.all(v)` returns `Scalar(v, v, v, v)` — commonly used for uniform fills (e.g., `cv.Scalar.all(-1)` for random per-keypoint colors in `drawKeypoints`). This project has no static method on the Scalar helper. Additionally, opencv.js's `Scalar` subclasses `Array` (so `scalar instanceof Array === true`); this project's returns a `Float64Array`.

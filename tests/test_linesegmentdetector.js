import { HSVtoRGB, Line, IMREAD_GRAYSCALE, IMREAD_COLOR, COLOR_BGR2Lab, COLOR_GRAY2BGR, LINE_AA, LSD_REFINE_ADV, LSD_REFINE_STD, LSD_REFINE_NONE, LineSegmentDetector, Mat, cvtColor, imread, imshow, line, split, waitKey, imwrite, } from 'opencv';
...
function main(input = 'tests/test_linesegmentdetector.jpg') {
  ...
  for(let ln of lines) {
    ...
    let color = [0, ((ln.angle + Math.PI) * 255) / (Math.PI * 2), 255];

    line(image, ln, color, 1, LINE_AA);
  }

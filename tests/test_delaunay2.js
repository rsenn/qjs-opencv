...
function locate_point(/*Mat&*/ img, /*Subdiv2D&*/ subdiv, /*Point2f*/ fp, /*Scalar*/ active_color) {
  ...
  if(e0 > 0) {
    ...
    do {
      ...
      if(subdiv.edgeOrg(e, org) > 0 && subdiv.edgeDst(e, dst) > 0) line(img, org, dst, active_color, 3, LINE_AA, 0);
...
function paint_voronoi(/*Mat&*/ img, /*Subdiv2D&*/ subdiv) {
  let facets = [],
    centers = [];
  ...
  for(let i = 0; i < facets.length; i++) {
    ...
    ifacets[0] = ifacet;
    drawPolylines(img, ifacets, true, Scalar(), 1, LINE_AA, 0);
    drawCircle(img, centers[i], 3, Scalar(), FILLED, LINE_AA, 0);
  }
}

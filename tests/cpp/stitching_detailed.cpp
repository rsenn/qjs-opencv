...
printUsage(char** argv) {
  cout << "Rotation model images stitcher.\n\n"
       ...
       << " img1 img2 [...imgN] [flags]\n\n"
          ...
          "\nCompositing Flags:\n"
          "  --warp "
          "(affine|plane|cylindrical|spherical|fisheye|stereographic|compressedPlaneA2B1|compressedPlaneA1.5B1|compressedPlanePortraitA2B1|"
...
parseCmdArgs(int argc, char** argv) {
  ...
  for(int i = 1; i < argc; ++i) {
    if(string(argv[i]) == "--help" || string(argv[i]) == "/?") {
      ...
      if(string(argv[i + 1]) == "no" || string(argv[i + 1]) == "voronoi" || string(argv[i + 1]) == "gc_color" || string(argv[i + 1]) == "gc_colorgrad" ||

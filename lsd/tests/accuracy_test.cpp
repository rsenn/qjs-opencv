...
checkRotatedRectangle() {
  ...
  Point center(rng.uniform(sz.width / 4, sz.width * 3 / 4), rng.uniform(sz.height / 4, sz.height * 3 / 4));
  Size rect_size(rng.uniform(sz.width / 8, sz.width / 6), rng.uniform(sz.height / 8, sz.height / 6));
  ...
  for(int i = 0; i < 4; i++) { cv::line(filledRect, vertices[i], vertices[(i + 1) % 4], Scalar(255), 3); }
...
checkLines() {
  ...
  vector<Vec4i> lines;
  Ptr<LSD> ls = createLSDPtr(::LSD_REFINE_NONE);

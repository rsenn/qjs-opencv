...
dominant_colors_grabber::dominant_colors_grabber(color_space cs, dist_type dt,
				unsigned colors_count, double colors_part):
_cs(cs), _dt(dt), _colors_count(colors_count), _colors_part(colors_part)
{
	
}
std::vector<cv::Scalar> dominant_colors_grabber::GetDomColors(cv::Mat img,
...
void dominant_colors_grabber::SetDistanceType(dist_type dt)
{
	if(dt != DT_UNDEFINED)
		_dt = dt;
}
dist_type dominant_colors_grabber::GetDistanceType()
{
	return _dt;
}
void dominant_colors_grabber::SetColorSpace(color_space cs)
{
	if(cs != CS_UNDEFINED)
		_cs = cs;
}
color_space dominant_colors_grabber::GetColorSpace()
{
	return _cs;
}
void dominant_colors_grabber::SetColorsCount(unsigned colors_count)
{
	if(colors_count != 0)
		_colors_count = colors_count;
}
unsigned dominant_colors_grabber::GetColorsCount()
{
	return _colors_count;
}
void dominant_colors_grabber::SetColorsPart(double colors_part)
{
	if(colors_part > 0)
		_colors_part = colors_part;
}
double dominant_colors_grabber::GetColorsPart()
{
	return _colors_part;
}
void dominant_colors_grabber::SetParam(cv::Vec3i param)
{
	_param = param;
}
cv::Vec3i dominant_colors_grabber::GetParam()
{
	return _param;
}
#pragma endregion

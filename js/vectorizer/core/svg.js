...
function styleAttrs(st) {
  ...
  if(st.stroke) {
    a.push(`stroke="${st.stroke}"`);
    a.push(`stroke-width="${fmt(st.strokeWidth)}"`);
  }
...
export function renderVectorData(vd, opts = {}) {
  ...
  const bg = vd.background ? `<rect width="${w}" height="${h}" fill="${vd.background}"/>\n  ` : '';
...
export function compose(canvasW, canvasH, placements, opts = {}) {
  ...
  const bg = opts.background ? `<rect width="${canvasW}" height="${canvasH}" fill="${opts.background}"/>\n` : '';

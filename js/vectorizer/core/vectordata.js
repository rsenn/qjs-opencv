...
export function emptyStyle(over = {}) {
  return Object.assign(
    { stroke: '#000000', strokeWidth: 1, fill: null, opacity: 1 },
    over,
  );
}
...
export function polyline(points, style) { return { kind: 'polyline', points, style: emptyStyle(style) }; }
export function polygon(points, style)  { return { kind: 'polygon', points, style: emptyStyle(style) }; }
export function path(subpaths, evenodd, style) { return { kind: 'path', subpaths, evenodd: !!evenodd, style: emptyStyle(style) }; }
export function line(a, b, style)       { return { kind: 'line', a, b, style: emptyStyle(style) }; }
export function circle(c, r, style)     { return { kind: 'circle', c, r, style: emptyStyle(style) }; }
export function ellipse(c, rx, ry, angle, style) { return { kind: 'ellipse', c, rx, ry, angle, style: emptyStyle(style) }; }
export function point(p, r, style)      { return { kind: 'point', p, r: r ?? 1, style: emptyStyle(style) }; }
...
export function flatten(shape, samples = 48) {
  const s = shape.style;
  switch (shape.kind) {
    case 'polyline': return { rings: [shape.points], closed: false, style: s };
    case 'polygon':  return { rings: [shape.points], closed: true, style: s };
    case 'path':     return { rings: shape.subpaths, closed: true, evenodd: shape.evenodd, style: s };
    case 'line':     return { rings: [[shape.a, shape.b]], closed: false, style: s };
    case 'point':    return { rings: [[shape.p]], closed: false, style: s, isPoint: true, r: shape.r };
    case 'circle':   return { rings: [sampleEllipse(shape.c, shape.r, shape.r, 0, samples)], closed: true, style: s };
    case 'ellipse':  return { rings: [sampleEllipse(shape.c, shape.rx, shape.ry, (shape.angle || 0) * Math.PI / 180, samples)], closed: true, style: s };
    default:         return { rings: [], closed: false, style: s };
  }
}

...
import {
  Mat, Size, Point, Rect, Scalar, resize, cvtColor, drawRect, drawLine,
  putText, getTextSize, COLOR_GRAY2BGR, FONT_HERSHEY_SIMPLEX, FILLED,
} from 'opencv.so';
...
function sc(c) { return new Scalar(c[0], c[1], c[2]); }
...
export class Canvas {
  ...
  textSize(str, scale = 0.5, thickness = 1) {
    try {
      ...
      if (r && r.width != null) return { w: r.width, h: r.height };
  ...
  paste(src, x, y, w, h) {
    ...
    if (ch === 1) { img = new Mat(); cvtColor(src, img, COLOR_GRAY2BGR); }
    const sw = src.cols ?? src.width, sh = src.rows ?? src.height;
    const s = Math.min(w / sw, h / sh);
    const dw = Math.max(1, Math.round(sw * s)), dh = Math.max(1, Math.round(sh * s));
    const ox = x + ((w - dw) >> 1), oy = y + ((h - dh) >> 1);
    const small = new Mat();

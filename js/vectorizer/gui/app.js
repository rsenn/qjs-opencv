...
import {
  namedWindow, destroyWindow, imshow, waitKey, WINDOW_AUTOSIZE,
} from 'opencv.so';
...
export class App {
  ...
  recreateWindow() {
    try { destroyWindow(WIN); } catch (_) {}
    namedWindow(WIN, WINDOW_AUTOSIZE);
    this.mouse = new Mouse(WIN);
    this.trackbars = new Trackbars(WIN);
  }

  get stage() { return this.stages[this.model.stage]; }
  ...
  next() { if (this.pipeline.canAdvance(this.model.stage)) this.goTo(this.model.stage + 1); }
  back() { this.goTo(this.model.stage - 1); }
  ...
  run() {
    ...
    while (running) {
      ...
      const clicks = this.events
        .filter((e) => e.event === MouseEvent.DOWN)
        .map((e) => ({ x: e.x, y: e.y }));
      ...
      else if (key === 9) this.next();                 // TAB -> next
  ...
  _drawChrome() {
    const cv = this.canvas, hud = this.hud;
    ...
    for (let i = 0; i < TABS.length; i++) {
      ...
      if (hud.button(cv, x, 7, tabW, 30, TABS[i], { active, disabled: i > this.model.stage }))
        if (i <= this.model.stage) this.goTo(i);
    }
    // nav buttons on the right
    if (hud.button(cv, this.W - 240, 7, 70, 30, '< Back', { disabled: this.model.stage === 0 }))
      this.back();
    const canNext = this.pipeline.canAdvance(this.model.stage) && this.model.stage < Stage.PROJECT;
    if (hud.button(cv, this.W - 162, 7, 70, 30, 'Next >', { disabled: !canNext, active: canNext }))
      this.next();
    if (hud.button(cv, this.W - 84, 7, 72, 30, 'Export', { active: this.model.stage === Stage.PROJECT }))
      this.export();

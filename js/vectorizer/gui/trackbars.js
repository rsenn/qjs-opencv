...
export class Trackbars {
  ...
  add(spec, current) {
    const count = spec.type === 'bool' ? 1 : spec.type === 'enum' ? spec.options.length - 1 : Math.max(1, Math.round((spec.max - spec.min) / (spec.step || 1)));
    const pos = this._valueToPos(spec, current ?? spec.default);
    try {
      createTrackbar(spec.label, this.win, pos, count, () => {});
    } catch(_) {}
  ...
  poll() {
    ...
    for(const s of this.specs) {
      let pos = s.last;
      try {
        pos = getTrackbarPos(s.spec.label, this.win);
      } catch(_) {}
      if(pos !== s.last) {
        s.last = pos;

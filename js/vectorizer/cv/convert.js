...
import {
  Mat, Rect, cvtColor, COLOR_BGR2GRAY,
  approxPolyDP, arcLength, mean,
} from 'opencv.so';
...
export function contoursToShapes(contours, opts = {}) {
  const {
    mode = 'stroke', style = {}, epsilon = 0, minPoints = 2, minArea = 0,
  } = opts;
  const shapes = [];
...
export function hierarchyToPaths(contours, hierarchy, opts = {}) {
  ...
  for (let i = 0; i < contours.length; i++) {
    ...
    shapes.push(path(subpaths.filter((s) => s.length >= 3), true, style));
  }
...
export function linesToShapes(lines, style = {}) {
  ...
  for (const l of lines || []) {
    let a, b;
    if (l && typeof l.x1 === 'number') { a = [l.x1, l.y1]; b = [l.x2, l.y2]; }
    else if (l && l.length >= 4) { a = [l[0], l[1]]; b = [l[2], l[3]]; }
    else continue;
...
export function bgrToHex(s) {
  const b = clamp255(s[0]), g = clamp255(s[1]), r = clamp255(s[2]);
...
export function meanColorMasked(src, mask) {
  try { return bgrToHex(mean(src, mask)); } catch (_) { return '#888888'; }
}
...
export function release(...mats) {
  for (const m of mats) {
    try { if (m && typeof m.delete === 'function') m.delete(); } catch (_) {}
  }
}
